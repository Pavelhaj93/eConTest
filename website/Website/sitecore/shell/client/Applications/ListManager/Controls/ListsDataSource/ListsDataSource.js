define(["sitecore", "jquery", "underscore", "/-/speak/v1/controls/searchdatasource.js"], function (Sitecore, $, _) {
  ...
  var model = searchDataSourceModel.extend({
    initialize: function (options) {
      ...
      var self = this;
      this.selectListOrFolder = function (selectedItemId, type, name) {
        var typeToCheck = type.toString().toLowerCase();
        switch (typeToCheck) {
          case self.get("folderType").toLowerCase():
            self.enterFolder(selectedItemId, name);
            break;
          case self.get("contactListType").toLowerCase():
            location.href = self.get("contactListPagePattern") + selectedItemId;
            break;
          case self.get("segmentedListType").toLowerCase():
            location.href = self.get("segmentedListPagePattern") + selectedItemId;
            break;
          default:
            break;
        }
    ...
    completed: function (items, context) {
      ...
      if (context.ancestorIds.length > 1) {
        var id = context.ancestorIds[context.ancestorIds.length - 2],
            topRow = { Id: id, itemId: id, Name: context.get("topFolderName"), Type: context.get("folderType"), Icon: context.get("folderIcon") };
  ...
  Sitecore.Factories.createComponent("ListsDataSource", model, view, ".sc-ListsDataSource");
});
