define([], function () {
  ...
  Checking.prototype.initialized = function () {
    this.CheckedValues = (typeof this.CheckedValues) === "string" ? JSON.parse(this.CheckedValues) : this.CheckedValues;
    this.on("change:DynamicData", checkDefault, this);
    this.on("change:CheckedValues", this.setCheckedValues, this);
    this.on("change:CheckedItems", this.setCheckedItems, this);

    if (this.hasData()) {
      checkDefault.call(this);
    }
  };

  function checkDefault() {
    if (this.DynamicData.length === 0 || this.CheckedValues.length === 0) {
      return;
    }

    this.checkByValues(this.CheckedValues);
  }
  ...
  Checking.prototype.checkAll = function () {
    this.check(this.DynamicData);
  };

  Checking.prototype.uncheckAll = function () {
    this.uncheck(this.DynamicData);
  };
