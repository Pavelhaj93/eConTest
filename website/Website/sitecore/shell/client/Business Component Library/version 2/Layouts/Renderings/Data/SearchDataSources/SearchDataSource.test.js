...
define(["mockjax"], function () {
  describe("Given a SearchDataSource component", function() {

    var app = Sitecore.Speak.app,
      sut1 = app.DataSource,
      sut2 = app.BindDataSource,
      sut3 = app.DataSource3,
      sut4 = app.DataSource2,
      sut8 = app.DataSource8,
      sut9 = app.EmptyDataSource;
    ...
    it("SearchQuery property should obtain value from rendering parameters", function() {
      expect(sut1.SearchQuery).toBe("test");
    });
    ...
    describe("when loadData executed", function () {
      ...
      it("should make request with searchQuery in parameters", function (done) {
        initMockjax(function (request) {
          expect(request.params.search).toBe(sut1.SearchQuery);
          done();
          return true;
        }, defaultResponse);
        sut1.loadData();
      ...
      it("should make request with SearchConfigItemId in parameters", function (done) {
        initMockjax(function (request) {

          expect(request.params.searchConfig).toBe(sut4.SearchConfigItemId);
          done();
          return true;
        }, defaultResponse);
        sut4.loadData();
      ...
      it("should make request with database in parameters", function (done) {
        initMockjax(function (request) {
          ...
          expect(request.params.sc_content).toBe(sut1.DatabaseName);
          done();
          return true;
        }, defaultResponse);
        sut1.loadData();
      ...
      it("should make request with sorting in parameters", function (done) {
        initMockjax(function (request) {

          expect(request.params.sorting).toBe(sut2.Sorting);
          done();
          return true;
        }, defaultResponse);
        sut2.loadData();
      ...
      it("should have facets in results", function (done) {
        initMockjax(null, defaultResponse);
        sut1.on("itemsChanged", function () {
          expect(sut1.Facets.length).toBe(3);
          done();
          return true;
        });
        sut1.loadData();
      ...
      it("should have HasData=true HasNoData=fasle HasMoreData=true", function (done) {
        initMockjax(null, defaultResponse);
        sut1.on("itemsChanged", function () {
          expect(sut1.HasData).toBe(true);
          expect(sut1.HasNoData).toBe(false);
          expect(sut1.HasMoreData).toBe(true);
          done();
          return true;
        });
        sut1.loadData();
      ...
      it("should make request with root in parameters", function (done) {
        initMockjax(function (request) {

          expect(request.params.root).toBe(sut2.SearchRootItemId);
          done();
          return true;
        }, defaultResponse);
        sut2.loadData();
    ...
    describe("if SelectedFacets defined", function () {
      sut8.SearchQuery = "some";
      it("should make request with according search query", function(done) {
        initMockjax(function(request) {

          expect(request.params.search).toBe(sut8.SearchQuery + " AND (calculateddimension:huge)");
          done();
          return true;
        }, defaultResponse);
        sut8.SelectedFacets = [{ "name": "Dimensions", "value": "calculateddimension:huge" }];
      });
    });

    describe("if next executed", function () {
      it("should make request with according page index", function (done) {
        initMockjax(function (request) {

          expect(request.params.pageIndex).toBe("1");
          done();
          return true;
        }, defaultResponse);
        sut8.next();
    ...
    describe("if server error happens", function () {
      it("should trigger error", function (done) {
        ...
        sut8.on("error", function(message) {
    ...
    /*
    describe("if authorization error happens", function () {
      it("should redirect to the login page", function (done) {
        initMockjax(function (request) {
          if (request.url == "/api/sitecore/Authentication/Logout") {
            expect(request.url).toBe("/api/sitecore/Authentication/Logout");
            done();
          }
          return true;
        }, error401Response);
    ...
    */
    ...
    describe("if no data", function () {
      it("should have HasData=false HasNoData=true HasMoreData=false", function () {
        expect(sut9.HasData).toBe(false);
        expect(sut9.HasNoData).toBe(true);
        expect(sut9.HasMoreData).toBe(false);
      });
    });

    describe("after the binded text changed", function () {
      it("should make request with new searchQuery value", function (done) {
        var newSearchValue = "new";
        initMockjax(function (request) {
          var search = request.params.search;
          expect(search).toBe(newSearchValue);
          done();
          return true;
        }, defaultResponse);
        app.TextBox2.Value = newSearchValue;
      });
    });
  });
});