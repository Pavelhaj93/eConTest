///#source 1 1 /sitecore/shell/client/Business Component Library/version 2/Layouts/Renderings/ListsAndGrids/ListControls/src/ListControl-Main.js
(function (Speak) {
  require.config({
    paths: {
  ...
  Speak.component(["collection", "selection", "checking", "scrollable", "ListControl/UserProfile", "ListControl/ViewModel", "ListControl/DetailList"], function (Collection, Selection, Checking, Scrollable, UserProfile, ViewModel, DetailList) {
    var view,
      viewModel,
      isSelectionRequired,
      ...
      extendItems = function () {
        this.on("hook:Items", function (hookEvent) {
      ...
      initListeners = function () {
        this.on("change:ViewMode change:Items change:IdFieldName change:ColumnDefinitionItems", this.render, this);
...
///#source 1 1 /sitecore/shell/client/Business Component Library/version 2/Layouts/Renderings/ListsAndGrids/ListControls/src/ListControl-ViewModel.js
(function (Speak) {
  define("ListControl/ViewModel", [], function () {
    var ViewModel = function (model) {
      ...
      function onScrollNearBottom(bool) {
        if (bool) this.scrollEnded();
      }
      ...
      model.on("change:IsScrollNearBottom", onScrollNearBottom, this);
    };
...
///#source 1 1 /sitecore/shell/client/Business Component Library/version 2/Layouts/Renderings/ListsAndGrids/ListControls/src/ListControl-DetailList.js
define("ListControl/DetailList",
  ...
  function (TemplateHelper, ResizableColumns, DetailListHeightFactory, ResponsiveColumns, DetailListHeader) {
    ...
    View.prototype.setAssociatedListPage = function (el, viewModel) {
      ...
      var scrollContainer = el.querySelector(".sc-listcontrol-body-wrapper"),
        associatedListPage = el.querySelector(".sc-associated-listpage"),
        link = el.querySelector(".sc-associated-listpage a");
    ...
    View.prototype.toggleCheckAll = function (viewmodel, event) {
      ...
      viewmodel.toggleCheckAll(isChecked);
    };

    View.prototype.toggleCheckOverSelect = function (viewmodel, event) {
      if (!viewmodel.isSelectionDisabled()) {
        var index = $(event.currentTarget).index();
        viewmodel.toggleCheckOverSelect(index);
      }
    };

    View.prototype.setChecked = function (el, indexes, allItemsAreChecked) {
      var $tbody = $(el).find(".sc-listcontrol-body tbody"),
        $checkedRows = $tbody.find("> tr.checked"),
        $headerCheckbox = $(el).find('.sc-listcontrol-checking-all input[type="checkbox"]');

      $checkedRows.removeClass("checked");
      $checkedRows.find('.sc-listcontrol-checking > input[type=checkbox]:checked').prop("checked", false);

      $headerCheckbox.prop("checked", allItemsAreChecked);
      ...
      _.each(indexes, function (index) {
        ...
        $row.find('.sc-listcontrol-checking > input[type=checkbox]').prop("checked", true);
      });
    };
    ...
    View.prototype.setSelected = function (el, index) {
      ...
      $tbody.find("> tr").eq(index).addClass("selected");
    };

    View.prototype.getViewModeSettings = function (viewModel, columnDefinitionItems) {
    ...
    View.prototype.setViewListeners = function (viewModel, el) {
      ...
      $(el).on("click.listcontrol:click", ".sc-listcontrol-item", this.click.bind(this, viewModel));
      $(el).on("click.listcontrol:toggleCheckOverSelect", ".sc-listcontrol-item", this.toggleCheckOverSelect.bind(this, viewModel));
      $(el).on("click.listcontrol:toggleCheck", ".sc-listcontrol-checking", this.toggleCheck.bind(this, viewModel));
      $(el).on("click.listcontrol:toggleCheckAll", ".sc-listcontrol-checking-all", this.toggleCheckAll.bind(this, viewModel));
    };
    ...
    View.prototype.updateDOM = function (oldScrollPosition, internalData, el, viewModel) {
      ...
      viewModel.setScrollElement(el.querySelector(".sc-listcontrol-body-wrapper"));
    ...
    View.prototype.render = function (viewModel, columnDefinitionItems, el) {
      ...
      var internalData = {
        ...
        Settings: this.getViewModeSettings(viewModel, columnDefinitionItems)
      };
...
///#source 1 1 /sitecore/shell/client/Business Component Library/version 2/Layouts/Renderings/ListsAndGrids/ListControls/src/ListControl-ResizableColumns.js
...
///#source 1 1 /sitecore/shell/client/Business Component Library/version 2/Layouts/Renderings/ListsAndGrids/ListControls/src/ListControl-TemplateHelper.js
(function (Speak) {
  require.config({
    paths: {
  ...
  define("ListControl/TemplateHelper", ["handlebars", "moment", "imageHelper"], function (handlebars, moment, imageHelper) {
    var getFixedNumber = function (value) {
      ...
      getColumnFieldValue = function (dataObj, prop, silent) {
        if (!dataObj.hasOwnProperty(this[prop]) && silent !== "silent") {
          throwError(prop + ' "' + this[prop] + '" was not found in data: ' + JSON.stringify(dataObj));
        }
      ...
      for (i in settingsObj) {
        if (settingsObj.hasOwnProperty(i) && (dataObj[i] || dataObj[i] === 0)) {
          ...
          if (option.indexOf(placeholderChar) > 0) {
            option = option.replace(new RegExp(placeholderChar, "g"), dataObj[i]);
          }
    ...
    TemplateHelper.prototype.getColumnPartial = function (context, item, options) {
      var name = "template-detaillist-column-" + this.ColumnType.toLowerCase();
      context = JSON.parse(JSON.stringify(context));

      context.ValueItem = item;

      return handlebars.partials[name](context, options);
    };
    ...
    TemplateHelper.prototype.getImageUrl = function (dataObj, prop) {
      ...
      var image = getColumnFieldValue.call(this, dataObj, prop);

      if (image.indexOf("<image") !== -1) {
        //TODO: $database is currently unavailble on the item, but needs to be implemented server side. 
        //TODO: Do we fail silently if no $database property is present or do we throw an error?
    ...
    TemplateHelper.prototype.getImageAlt = function (dataObj, altProp, imageProp) {
      ...
      var alt = getColumnFieldValue.call(this, dataObj, altProp, "silent"),
          image = getColumnFieldValue.call(this, dataObj, imageProp);
    ...
    TemplateHelper.prototype.getColumnFieldDateTimeValue = function (dataObj, prop) {
      // Gets the value of DataFieldName property, if it exists, and converts it using MomentJS

      var rawValue = handlebars.helpers["ListControl:GetColumnFieldValue"].call(this, dataObj, prop);
      var value = moment(rawValue, this.ValueFormat).locale(Speak.Context.current().language).format(this.DisplayFormat);
...
///#source 1 1 /sitecore/shell/client/Business Component Library/version 2/Layouts/Renderings/ListsAndGrids/ListControls/src/ListControl-ResponsiveColumns.js
...
///#source 1 1 /sitecore/shell/client/Business Component Library/version 2/Layouts/Renderings/ListsAndGrids/ListControls/src/ListControl-DetailList-Header.js
...
///#source 1 1 /sitecore/shell/client/Business Component Library/version 2/Layouts/Renderings/ListsAndGrids/ListControls/src/ListControl-UserProfile.js
(function (Speak) {
  define("ListControl/UserProfile", ["/sitecore/shell/client/Business Component Library/version 2/Assets/lib/scUserProfile.js"], function () {
    ...
    var stateProperties = [
          ...
          "Sorting",
          "ColumnWidths",
          { "ColumnDefinitionItems": setColumnDefinitionItemsState }
        ],

    ommissionsOnCompare = ["ColumnWidth", "Sorting", "ColumnStoredWidth"];

    function setColumnDefinitionItemsState() {
      if (Array.isArray(this.ColumnDefinitionItems) && this.ColumnDefinitionItems.every(equalToColDefState.bind(this))) {
        this.ColumnDefinitionItems = this.UserProfileState["ColumnDefinitionItems"];
      }
    }

    function equalToColDefState(item) {
      omitProps = function(item) {
        return _.omit(_.clone(item), ommissionsOnCompare);
      },
      stateItems = this.UserProfileState["ColumnDefinitionItems"].map(function(obj) {
        return omitProps(obj);
      }),
      prunedItem = omitProps(item);

      return !stateItems.every(function (stateItem) {
        return !_.isEqual(prunedItem, stateItem);
      });
    }
    ...
    var saveHandler = Speak.module('userProfile'),
    ...
    UserProfile.prototype.initialized = function () {
      ...
      if (this.UserProfileState) {
        this.UserProfileState = typeof this.UserProfileState === "string" ? JSON.parse(this.UserProfileState) : this.UserProfileState;
...
///#source 1 1 /sitecore/shell/client/Business Component Library/version 2/Layouts/Renderings/ListsAndGrids/ListControls/src/Height/ListControl-DetailList-HeightFactory.js
...
///#source 1 1 /sitecore/shell/client/Business Component Library/version 2/Layouts/Renderings/ListsAndGrids/ListControls/src/Height/ListControl-DetailList-Height-Row.js
...
///#source 1 1 /sitecore/shell/client/Business Component Library/version 2/Layouts/Renderings/ListsAndGrids/ListControls/src/Height/ListControl-DetailList-Height-Default.js
...
///#source 1 1 /sitecore/shell/client/Business Component Library/version 2/Layouts/Renderings/ListsAndGrids/ListControls/src/Height/ListControl-DetailList-Height-Inherited.js
