    ...
    public class SigningClient
    {
        public FileToBeDownloaded SendDocumentsForMerge(FileToBeDownloaded pdfFile, byte[] signFile)
        {
            using (var api = this.InitApi())
            {
                ...
                return new FileToBeDownloaded()
                {
                    FileContent = signingResult.outputPDF.binaryData.ToList(),
                    FileName = pdfFile.FileName,
                    FileNumber = pdfFile.FileNumber,
                    FileType = pdfFile.FileType,
                    Index = pdfFile.Index,
                    SignRequired = pdfFile.SignRequired,
                    SignedVersion = true,
                    TemplAlcId = pdfFile.TemplAlcId
                };
            }
        }
