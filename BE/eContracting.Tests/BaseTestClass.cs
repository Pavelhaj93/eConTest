...
namespace eContracting.Tests
{
    public abstract class BaseTestClass
    {
        ...
        public OffersModel CreateOffer()
        {
            return CreateOffer(this.CreateGuid());
        }

        public OffersModel CreateOffer(Guid guid)
        {
            return CreateOffer(guid.ToString("N").ToUpperInvariant());
        }

        public OffersModel CreateOffer(int version)
        {
            return CreateOffer(this.CreateGuid(), false, version, "3");
        }

        public OffersModel CreateOffer(string guid)
        {
            return CreateOffer(guid, false, 2);
        }

        public OffersModel CreateOffer(string guid, bool isAccepted, int version)
        {
            return CreateOffer(guid, isAccepted, version, "3");
        }

        public OffersModel CreateOffer(string guid, bool isAccepted, int version, string state)
        {
            return CreateOffer(guid, isAccepted, version, state, "31.12.2021", null);
        }

        public OffersModel CreateOffer(bool isAccepted, int version)
        {
            return CreateOffer(this.CreateGuid(), isAccepted, version, "3");
        }

        public OffersModel CreateOffer(bool isAccepted, int version, string state)
        {
        ...
        /// <returns></returns>
        public OffersModel CreateOffer(string guid, bool isAccepted, int version, string state, string validTo, OfferAttributeModel[] attributes)
        {
            ...
            return new OffersModel(offer.Guid, new[] { offer });
        }

        public UserCacheDataModel CreateAnonymousUser(OffersModel offer)
        {
        ...
        public UserCacheDataModel CreateTwoSecretsUser(OffersModel offer)
        {
            var user = new UserCacheDataModel();
        ...
        public UserCacheDataModel CreateCognitoUser(OffersModel offer)
        {
